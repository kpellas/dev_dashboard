...
app.post('/api/sessions', async (req, res) => {
  ...
  try {
    ...
    // Generate prompt
    const prompt = generateSessionPrompt(session, project);
    await fs.writeFile(
      path.join(sessionsPath, `${sessionId}-prompt.md`),
      prompt
    );
...
// Helper function to generate session prompt
function generateSessionPrompt(session, project) {
  return `# Development Session: ${session.projectName} - ${session.sprintName}
...
5. Commit changes
6. Update item statuses

---
*Generated by Dev Dashboard*
`;
}
